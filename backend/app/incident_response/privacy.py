"""Privacy controls, multi-tenancy isolation, and data redaction for Incident Response (Task 61)."""
...
class IncidentPrivacyManager:
    """Manages context sanitization, PII masking, and multi-tenant isolation for incident data."""
    ...
    def sanitize_payload(self, payload: dict[str, Any], tenant_id: str | None = None) -> dict[str, Any]:
        ...
        for key, val in payload.items():
            if key.startswith("__") or key.lower() in (
                ...
                sanitized[key] = self.mask_pii(scrub_incident_secrets(val))
                ...
                sanitized[key] = [
                    ...
                    else (self.mask_pii(scrub_incident_secrets(item)) if isinstance(item, str) else item)
                    for item in val
                ]
            else:
                sanitized[key] = val
    ...
    def validate_tenant_access(
        ...
        """Enforce strict multi-tenant boundary for incident data access."""
        if is_admin or incident_tenant.lower() in ("system", "global"):
            return True
        return incident_tenant.lower() == current_tenant.lower()


incident_privacy_manager = IncidentPrivacyManager()
