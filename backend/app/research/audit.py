"""Tamper-evident structured audit trail for Knowledge Synthesis & Research Intelligence Engine (Task 63)."""
...
from app.research.safety import scrub_research_secrets
...
class ResearchAuditor:
    ...
    def record_event(
        ...
        logger.info(
            "RESEARCH_AUDIT_LOG: seq=%d event=%s actor=%s hash=%s",
            sequence_num,
            event_type,
            actor,
            current_hash,
    ...
    def record_action(
        ...
        actor: str = "research_engine",
        **kwargs: Any,
    ) -> AuditRecord:
        """Convenience method for recording an action with audit chaining."""
    ...
    def get_events(
        ...
        """Retrieve audit log entries filtered by session, source, or claim."""
        ...
        if claim_id:
            results = [e for e in results if e.get("claim_id") == claim_id]
    ...
    def get_audit_trail(
        ...
        return self.get_events(session_id=session_id, source_id=source_id, claim_id=claim_id, limit=limit)
    ...
    def _sanitize_dict(self, data: dict[str, Any]) -> dict[str, Any]:
        ...
        for k, v in data.items():
            if isinstance(v, str):
                sanitized[k] = scrub_research_secrets(v)
                ...
                sanitized[k] = [
                    ...
                    else (scrub_research_secrets(x) if isinstance(x, str) else x)
                    for x in v
                ]
            else:
                sanitized[k] = v
